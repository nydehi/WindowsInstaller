// <file>
//     <copyright see="2003,2004,2005,2007 Youseful Software"/>
//     <license see="MPL"/>
//     <owner name="Youseful Software" email="support@youseful.com"/>
//     <version value="0.95"/>
// </file>
// Original Author: Joseph J Mele (jmele@youseful.com)
//
using System;
using System.Text;
using System.Runtime.InteropServices;
using SRIC = System.Runtime.InteropServices.ComTypes;
using Youseful.WinAPI;
using Youseful.WinAPI.Interop;
using Youseful.Installer.Base;
//Not supported in 3.1 or earlier
//http://msdn2.microsoft.com/en-us/library/aa816408.aspx
#region Update Log
/*************************************************************************
 *  version .94
 * ==============================
 * -------- Removed Redundant INSTALLMESSAGE_XXX
 * -------- Added MsiSetInternalUI
 * 
 *  version .93
 * ==============================
 * -------- Added AdvertiseFeature Functions
 * -------- Fixed parameter types concering out and ref.
 * -------- replaced string with stringbuilder when appropiate
 * -------- Upgraded to .Net 2.0
 * -------- Upgraded to Windows Installer 4.0
 * 
 * version .92
 * ==============================
...
namespace Youseful.Installer.Nativemsi
{
	...
	public class NativemsiAttribute : System.Attribute
	{
        ...
        public WindowsInstallerVersion DeprecatedVersion
        {
            get
            { return _Version; }
            //set	{ _Version = value;}
	...
	public class Msidll
	{
		...
		public enum	MsiCustomActionType : int
		{
			// executable types
			/// <summary>
            /// Source = Key to a row in the binary table
		...
		{
			FATALEXIT      = 0x00000000L, // premature termination, possibly fatal OOM
			ERROR          = 0x01000000L, // formatted error message
			WARNING        = 0x02000000L, // formatted warning message
			USER           = 0x03000000L, // user request message
			INFO           = 0x04000000L, // informative message for log
			FILESINUSE     = 0x05000000L, // list of files in use that need to be replaced
			RESOLVESOURCE  = 0x06000000L, // request to determine a valid source location
			OUTOFDISKSPACE = 0x07000000L, // insufficient disk space message
			ACTIONSTART    = 0x08000000L, // start of action: action name & description
			ACTIONDATA     = 0x09000000L, // formatted data associated with individual action item
			PROGRESS       = 0x0A000000L, // progress gauge info: units so far, total
			COMMONDATA     = 0x0B000000L, // product info for dialog: language Id, dialog caption
			INITIALIZE     = 0x0C000000L, // sent prior to UI initialization, no string data
			TERMINATE      = 0x0D000000L, // sent after UI termination, no string data
			SHOWDIALOG     = 0x0E000000L, // sent prior to display or authored dialog or wizard
		};
        /// <summary>
       ...
       //#endif // (_WIN32_MSI >= 300)
		...
		public enum MSIDBSTATE
		{
			...
			READ     = 0,  //
            /// <summary>
            /// database readable and updatable
            /// </summary>
			WRITE    = 1  // 
		} ;
        ...
        /// <summary>
        /// The <c>MsiAdvertiseProduct</c> function generates an advertise script or 
        /// advertises a product to the computer. The MsiAdvertiseProduct function enables
        /// the installer to write to a script the registry and shortcut information used 
        /// to assign or publish a product. The script can be written to be consistent with
        /// a specified platform by using MsiAdvertiseProductEx. 
        ///</summary>
        /// <param name="PackagePath"> [in] The full path to the package of the product being advertised.</param>
        /// <param name="ScriptfilePath">[in] The full path to script file that will be created with the
        /// advertise information. To advertise the product locally to the computer,
        /// set ADVERTISEFLAGS_MACHINEASSIGN or ADVERTISEFLAGS_USERASSIGN.
        /// </param>
        /// <param name="Transforms">[in] A semicolon-delimited list of transforms to be applied. The list of 
        /// transforms can be prefixed with the @ or | character to specify the secure caching of transforms. 
        /// The @ prefix specifies secure-at-source transforms and the | prefix indicates secure full path
        /// transforms. For more information, see Secured Transforms. This parameter may be null.
        /// </param>
        /// <param name="Language"> [in] The installation language to use if the source supports multiple 
        /// languages.
        /// </param>
        /// <returns></returns>
        /// <remarks>Please refer to the MSDN documentation for more information.</remarks>
        [WinAPI(OS.None, FileName = "Msi.h", UpgradeVersion = OS.None)]
        [Nativemsi(WindowsInstallerVersion.version10, Context = msiContext.InstallTime)]
        [DllImport("msi", CharSet = CharSet.Auto)]
        extern static public int MsiAdvertiseProduct(
            string PackagePath,
        ...
        /// languages.
        /// </param>
        /// <param name="Platform"> [in] Bit flags that control for which platform the installer should create
        /// the script. This parameter is ignored if szScriptfilePath is null. If dwPlatform is zero (0), then 
        /// the script is created based on the current platform. This is the same functionality
        /// as MsiAdvertiseProduct. If Platform is 1 or 2, the installer creates script for the specified
        /// platform. 
        /// </param>
        /// <param name="Options">[in] Bit flags that specify extra advertisement options. Nonzero value 
        /// is only available in Windows Installer versions shipped with Windows Server 2003 and
        /// Windows XP SP1 and later.
        /// </param>
        /// <returns></returns>
        /// <remarks>Please refer to the MSDN documentation for more information.</remarks>
        [WinAPI(OS.None, FileName = "Msi.h", UpgradeVersion = OS.None)]
        [Nativemsi(WindowsInstallerVersion.version15, Context = msiContext.InstallTime)]
        [DllImport("msi", CharSet = CharSet.Auto)]
        extern static public int MsiAdvertiseProductEx(
            string PackagePath,
            string ScriptfilePath,
            string Transforms,
            int Language,
		...
		/// <returns></returns>
         [WinAPI(OS.Win2k|OS.WinXP|OS.WinME|OS.Win2k3|OS.WinVista,
			 FileName = "msi.h",UpgradeVersion = OS.None)]
        ...
        /// <returns></returns>
         [WinAPI(OS.Win2k | OS.WinXP | OS.WinME | OS.Win2k3 | OS.WinVista,
             FileName = "msi.h", UpgradeVersion = OS.None)]
		[Nativemsi(WindowsInstallerVersion.version20,Context = msiContext.DesignTime | msiContext.RunTime)]
        [DllImport("msi", CharSet = CharSet.Auto)]
		...
		public static extern int MsiGetProductCode(
			string    Component,
             [MarshalAs(UnmanagedType.LPArray, SizeConst = 39)] byte[] GUIDString);   
		...
		/// <summary>
        /// function enumerates through one or all the instances of products
        /// that are currently advertised or installed in the specified contexts. 
        /// This function supersedes MsiEnumProducts
		/// </summary>
        /// <param name="ProductCode">0-based index into registered products</param>
        /// <param name="UserSid">buffer of char count: 39 (size of string GUID)</param>
        /// <param name="Context">0-based index into registered products</param>
        /// <param name="Index">buffer of char count: 39 (size of string GUID)</param>
        /// <param name="InstalledProductCode">0-based index into registered products</param>
        /// <param name="InstalledContext">buffer of char count: 39 (size of string GUID)</param>
        /// <param name="Sid">0-based index into registered products</param>
        /// <param name="Sid_Length">buffer of char count: 39 (size of string GUID)</param>
		/// <returns></returns>
        [WinAPI(OS.Win2k | OS.WinXP | OS.WinME | OS.Win2k3,
             FileName = "msiQuery.h", UpgradeVersion = OS.None)]
        [Nativemsi(WindowsInstallerVersion.version30,
             Context = msiContext.InstallTime | msiContext.RunTime)]
        [DllImport("msi", CharSet = CharSet.Auto)]
        public static extern int MsiEnumProductsEx(
            string ProductCode,
             [MarshalAs(UnmanagedType.LPArray, SizeConst = 39)] byte[] UserSid,
            Msidll.MSIINSTALLCONTEXT Context,
            uint Index,
            [MarshalAs(UnmanagedType.LPArray, SizeConst = 39)] byte[] InstalledProductCode,//The MSDN doc is wrong see msi.h
            out Msidll.MSIINSTALLCONTEXT InstalledContext,
             [MarshalAs(UnmanagedType.LPArray, SizeConst = 39)] byte[] Sid,
            ref int Sid_Length);
        ...
        public static extern int MsiEnumProducts(
            int iProductIndex,
             [MarshalAs(UnmanagedType.LPArray, SizeConst = 39)] byte[] ProductBuffer); 
		...
		public static extern int MsiEnumClients(
			...
			int     iProductIndex,    // 0-based index into client products
             [MarshalAs(UnmanagedType.LPArray, SizeConst = 39)] byte[] ProductBuffer);    // buffer of char count: 39 (size of string GUID)
	

		// Enumerate the qualifiers for an advertised component.
		[DllImport("msi", CharSet=CharSet.Auto)]
		public static extern int MsiEnumComponentQualifiers(
			string   Component,         // generic component ID that is qualified
		...
		public static extern int MsiSummaryInfoSetProperty(IntPtr SummaryInfo,
			...
			ref	SRIC.FILETIME pftValue,      // pointer to filetime value, used only if datetime property
		...
		public static extern int MsiSummaryInfoGetProperty(IntPtr SummaryInfo,
            ...
            ref	SRIC.FILETIME pftValue,      // returned datetime property data
		...
		public delegate int MsiInstallUIHandler(IntPtr context,
			uint messageType,
            [MarshalAs(UnmanagedType.LPTStr)] string message);//message can be unicode or ansi so LPTStr not LPStr
		#endregion	Delegates
		...
		[Nativemsi(WindowsInstallerVersion.version10, Context = msiContext.InstallTime)]
		[DllImport("msi", CharSet=CharSet.Auto)]              
		public static extern INSTALLUILEVEL MsiSetpublicUI (long dwUILevel,ref IntPtr winhandle);        

		/// <summary>The <c>MsiSetExternalUI</c> function enables an external user-interface handler.
        /// This external UI handler is called before the normal public user-interface handler. 
        /// The external UI handler has the option to suppress the public UI by returning a non-zero
        /// value to indicate that it has handled the messages.</summary>
		/// <param name="handler">The <see cref="MsiInstallUIHandler"/> handler delegate.</param>
		/// <param name="filter">Specifies which messages to handle using the external message handler. 
        /// If the external handler returns a non-zero result, then that message will not be sent to the UI, 
        /// instead the message will be logged if logging has been enabled. See <see cref="MsiEnableLog"/>.</param>
        ...
        /// <summary>The <c>MsiSetInternalUI</c> function enables the installer's internal user interface.
        /// Then this user interface is used for all subsequent calls to user-interface-generating installer
        /// functions in this process.</summary>
        /// <param name="level">Specifies the level of complexity of the user interface.</param>
        /// <param name="parentWindow">Pointer to a window. This window becomes the owner of any user interface created.
        /// A pointer to the previous owner of the user interface is returned. If this parameter is <c>null</c>, 
        /// the owner of the user interface does not change.</param>
        ...
        /// <remarks>Please refer to the MSDN documentation for more information.</remarks>
        [DllImport("msi", CharSet = CharSet.Auto)]
        extern static public INSTALLUILEVEL MsiSetInternalUI(INSTALLUILEVEL level,
        ...
        /// <summary>The <c>MsiEnumPatchesEx</c> function enumerates all patches in a specific context
        /// or across all contexts. Patches already applied to products are enumerated. 
        /// Patches that have been registered but not yet applied to products are also enumerated.
        ///</summary>
        /// <param name="ProductCode">A null-terminated string that specifies the ProductCode GUID 
        /// of the product whose patches are enumerated. If non-null, patch enumeration is restricted to 
        /// instances of this product under the user and context specified by szUserSid and dwContext.
        /// If null, the patches for all products under the specified context are enumerated.
        /// </param>
        /// <param name="UserSid">
        /// A null-terminated string that specifies a security identifier (SID) 
        /// that restricts the context of enumeration. The special SID string s-1-1-0 (Everyone) specifies 
        /// enumeration across all users in the system. A SID value other than s-1-1-0 is considered a user SID 
        /// and restricts enumeration to that user. When enumerating for a user other than current user, any patches 
        /// that were applied in a per-user-unmanaged context using a version less than Windows Installer version 3.0, 
        /// are not enumerated. This parameter can be set to null to specify the current user.
        /// </param>
        /// <param name="Context">
        /// Restricts the enumeration to one or a combination of contexts. This parameter can be any one or 
        ...
        /// </param>
        /// <param name="Index">
        /// The index of the patch to retrieve. This parameter must be zero for the first call to the MsiEnumPatchesEx 
        /// function and then incremented for subsequent calls. The dwIndex parameter should be incremented only 
        /// if the previous call returned <see cref="MsiError.Success"/>ERROR_SUCCESS.
        /// </param>
        /// <param name="PatchCode">
        /// An output buffer to contain the GUID of the patch being enumerated. The buffer should be large enough 
        /// to hold the GUID. This parameter can be null.
        /// </param>
        /// <param name="TargetProductCode">
        /// An output buffer to contain the ProductCode GUID of the product that receives this patch. 
        /// The buffer should be large enough to hold the GUID. This parameter can be null.
        /// </param>
        /// <param name="TargetProductContext">
        /// Returns the context of the patch being enumerated. The output value can be
        /// MSIINSTALLCONTEXT_USERMANAGED, MSIINSTALLCONTEXT_USERUNMANAGED, 
        /// or MSIINSTALLCONTEXT_MACHINE. This parameter can be null.
        /// </param>
        /// <param name="TargetUserSid">
        /// An output buffer that receives the string SID of the account under which this patch instance exists. 
        /// This buffer returns an empty string for a per-machine context.
        /// This buffer should be large enough to contain the SID. If the buffer is too small, the function 
        /// returns ERROR_MORE_DATA and sets *pcchTargetUserSid to the number of TCHAR in the value, not including 
        /// the terminating NULL character.
        ///If the szTargetUserSid is set to NULL and pcchTargetUserSid is set to a valid pointer, the function 
        ///returns ERROR_SUCCESS and sets *pcchTargetUserSid to the number of TCHAR in the value, not including 
        ///the terminating NULL character. The function can then be called again to retrieve the value,
        ///with szTargetUserSid buffer large enough to contain *pcchTargetUserSid + 1 characters.
        ///If szTargetUserSid and pcchTargetUserSid are both set to NULL, the function returns ERROR_SUCCESS
        ///if the value exists, without retrieving the value.
        /// </param>
        /// <param name="TargetUserSid_Length">
        /// Set to the number of characters copied to <c>transform</c> upon an unsuccessful return of the function.
        /// </param>
        /// <returns>
        /// <para><see cref="ERROR_ACCESS_DENIED"/>
        /// The function fails trying to access a resource with insufficient privileges.
        /// </para>
        /// <para><see cref="ERROR_BAD_CONFIGURATION"/>  The configuration data is corrupt.</para>
        /// <para><see cref="ERROR_INVALID_PARAMETER"/>  An invalid parameter is passed to the function.</para>
        /// <para><see cref="ERROR_NO_MORE_ITEMS"/>  There are no more patches to enumerate.</para>
        /// <para><see cref="ERROR_SUCCESS"/> The patch is successfully enumerated.</para>
        /// <para><see cref="ERROR_UNKNOWN_PRODUCT"/> 
        /// The product that szProduct specifies is not installed on the computer in the specified contexts.
        /// </para>
        /// <para><see cref="ERROR_MORE_DATA"/>
        /// This is returned when pcchTargetUserSid points to a buffer size less than required to copy the SID.
        /// In this case, the user can fix the buffer and call <c>MsiEnumPatchesEx</c> again for the same index value.
        /// </para>
        /// </returns>
        /// <remarks>
        /// Non-administrators can enumerate patches within their visibility only.
        /// Administrators can enumerate patches for other user contexts.
        /// </remarks>
        [WinAPI(OS.None,FileName = "Msi.h",UpgradeVersion = OS.WinXP)]
		[Nativemsi(WindowsInstallerVersion.version30, Context = msiContext.InstallTime)]
		[DllImport("msi" , CharSet = CharSet.Auto)]
		extern static public int MsiEnumPatchesEx(string ProductCode,string UserSid,
			uint Context, uint Filter,uint Index,
			[MarshalAs(UnmanagedType.LPArray, SizeConst=39)] byte[] PatchCode,
			[MarshalAs(UnmanagedType.LPArray, SizeConst=39)] byte[] TargetProductCode,
			ref MSIINSTALLCONTEXT TargetProductContext,
			string TargetUserSid,ref uint TargetUserSid_length);
